...
class YQMagics(Magics):
    ...
    async def yq_execute_query(self,
        ...
        if YQMagics.Sa_info is not None:
            yq.set_service_account_key(YQMagics.Sa_info)
    ...
    @line_magic
    @magic_arguments()
    @argument("--sa-file", help="Path to Yandex Cloud static credentials file", type=str)  # noqa
    ...
    def yq_settings(self, line):
        ...
        if args.sa_file is not None:
            sa_file = args.sa_file.strip()
            with open(sa_file, "r") as sa_file:
                sa_info = sa_file.read()
            YQMagics.Sa_info = sa_info
